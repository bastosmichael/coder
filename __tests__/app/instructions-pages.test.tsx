...
const { getInstructionsByProjectId, getInstructionById } = require('../db/queries/instructions-queries') as any
const InstructionListMock = require('../components/instructions/instruction-list').InstructionsList as jest.Mock
const InstructionMock = require('../components/instructions/instruction').Instruction as jest.Mock
...
describe('instruction pages', () => {
  it('lists instructions for project', async () => {
    ...
    const Page = await InstructionListPage({ params: Promise.resolve({ workspaceId: 'w', projectId: 'p' }) } as any)
    render(Page as any)
    expect(InstructionListMock).toHaveBeenCalledWith({ instructions: ['a'] }, {})
  })

  it('renders single instruction', async () => {
    getInstructionById.mockResolvedValue({ id: '1', name: 'test' })
    const Page = await InstructionPage({ params: Promise.resolve({ workspaceId: 'w', projectId: 'p', id: '1' }) } as any)
    render(Page as any)
    expect(InstructionMock).toHaveBeenCalledWith({ instruction: { id: '1', name: 'test' } }, {})
  ...
  it('renders edit instruction page', async () => {
    ...
    const Page = await EditInstructionPage({ params: Promise.resolve({ workspaceId: 'w', projectId: 'p', id: '1' }) } as any)
    render(Page as any)
