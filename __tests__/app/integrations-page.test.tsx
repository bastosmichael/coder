...
jest.mock('../components/integrations/integrations', () => ({
  Integrations: jest.fn(({ isGitHubConnected }) => <div>integrations {String(isGitHubConnected)}</div>)
}))
...
const IntegrationsMock = require('../components/integrations/integrations').Integrations as jest.Mock
...
describe('IntegrationsPage', () => {
  it('handles missing workspace', async () => {
    ...
    const Page = await IntegrationsPage({ params: Promise.resolve({ workspaceId: 'w', projectId: 'p' }) } as any)
    render(Page as any)
  ...
  it('renders integrations with connection state', async () => {
    ...
    const Page = await IntegrationsPage({ params: Promise.resolve({ workspaceId: 'w', projectId: 'p' }) } as any)
    render(Page as any)
    expect(IntegrationsMock).toHaveBeenCalledWith({ isGitHubConnected: true }, {})
  })
})
