...
const { getIssuesByProjectId, getIssueById } =
  require("../../db/queries/issues-queries") as any
const { getProjectById } = require("../../db/queries/projects-queries") as any
const { getInstructionsForIssue } =
  require("../../db/queries/issues-to-instructions-queries") as any
const IssuesListMock = require("../../components/issues/issues-list")
  .IssuesList as jest.Mock
const IssueViewMock = require("../../components/issues/issue-view")
  .IssueView as jest.Mock
...
describe("issue pages", () => {
  it("lists project issues", async () => {
    ...
    render(Page as any)
    expect(IssuesListMock).toHaveBeenCalledWith(
      { issues: ["i"], projectId: "p" },
      {}
    )
  })
  ...
  it("renders edit issue page", async () => {
    getIssueById.mockResolvedValue({ id: "1" })
    const Page = await EditIssuePage({
      params: Promise.resolve({
        workspaceId: "w",
        projectId: "p",
        issueId: "1"
  ...
  it("renders create issue page", async () => {
    const Page = await CreateIssuePage({
      params: Promise.resolve({ workspaceId: "w", projectId: "p" })
    } as any)
    render(Page as any)
